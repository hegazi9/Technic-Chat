...
const Login = ({ navigation, toast }) => {
  ...
  const loginFun = () => {
    if (username == '') { ref_input1.current.focus(); setMessageError("User name is empty") }
    else if (password == '') { ref_input2.current.focus(); setMessageError("Password is empty") }
    else {
      setMessageError('')
      //  alert(users.length)
      users.length > 0 ?
        users.map(item => {
          if (item.username == username && item.password == password) {
            ...
            return true;
          } else toast.current.show('Username or Password incorrect', 3000)
  ...
  return (
    <View>
      ...
      <View style={styles.loginContainer}>
        ...
        <Text style={styles.lightText} >Login with</Text>
